...
%     Q4: the m* closed form is REFUTED at small frac(eta K) (see the
%     correction note below); m* is now defined as the argmax
...
\[
  D(m)=\frac{q^{j}\bigl(\nu^{m}/K-1\bigr)}{\eta(\nu^{m}-1)-m},
  \qquad
  m^{\ast}\in\operatorname*{arg\,max}_{m\ge1}D(m)
\]
(the supremum is attained at finite $m^{\ast}$: $D(m)\to q^{j}/(K\eta)$
from above as $m\to\infty$), $t^{\ast}=j+m^{\ast}$, and let
\[
  D=\max\Bigl\{\frac{q^{j}}{K\eta},\ D(m^{\ast})\Bigr\},
  \qquad
  W_K(\eta)=1-q^{j}+(K-j)D .
\]
% Q4 correction (ledger T12): the earlier closed-form guess
% m* = ceil(eta K) - 1 is REFUTED by exact rationals when frac(eta K) is
% small (e.g. K=5, eta=2001/1000: true argmax 9, guess 10; four
% counterexamples, results/Q2_symbolic.md), and at those corner points the
% family built from the guessed m* is infeasible (r(t*) < 0).  The argmax
% form above is what every script in results/ uses; the candidate rule
% m* = min{m >= 1 : nu^m (eta K - 1 - m) <= K(eta - 1)} <= ceil(eta K) - 1
% stays [CONJECTURE].
