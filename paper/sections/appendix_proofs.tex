...
% ---------------------------------------------------------------------------
\subsection{Supplementary theorem: queries of arbitrary size}
...
\begin{theorem}[Bounded-query hardness, unrestricted query size]
\label{thm:hardness-anysize}
Let $K\ge3$, $\eta>1$ and $n\ge4K(t^{\ast}+K)$, which forces $n\ge8K^{2}$.
For every deterministic algorithm
that makes at most $Q\le n^{2}/\bigl(2K^{2}(t^{\ast}+K)^{2}\bigr)$ queries to
$\tilde f$, of arbitrary size, and returns a set of size at most $K$, there is
a $K$-set $O$ for which the instance $(f,\tilde f)$ has monotone submodular
$f$, single-element error exactly $\eta$, $O^{\ast}=O$, and the output
$T_{\mathcal A}$ of the algorithm satisfies
\[
  \frac{f(T_{\mathcal A})}{f(O^{\ast})}\;\le\;W_K(\eta)
  \;=\;V_j(\eta)+(K-j)\Bigl(D-\frac{q^{j}}{K\eta}\Bigr).
\]
For randomized algorithms the same bound holds in expectation up to an
additive $K/n+QK^{2}(t^{\ast}+K)^{2}/(2n^{2})$.
\end{theorem}

Since $t^{\ast}+K\le(2+\eta)K+1$, the admissible budget is
$Q=\Omega\bigl(n^{2}/((2+\eta)^{2}K^{4})\bigr)$, that is $n^{2-o(1)}$ and not
every polynomial.  The exponent $2$ is a property of this family rather than
