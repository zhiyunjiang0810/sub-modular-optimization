...
algorithm's randomness only.  The additive term vanishes only in the joint
limit $n/K^{5}\to\infty$; at fixed $n$ it does not.  The combination with
the ceiling as $\min\{U_K(\eta),1/\eta\}$ is stated for deterministic
algorithms only.
% M2.6 (J5): joint-limit condition and the deterministic-only scope of the
% min combination made explicit; ledger T10b.
