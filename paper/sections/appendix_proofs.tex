...
of collapsing the whole bound to $0$.  For a run stopped after $K^{*}<K$
steps the product ranges over the executed steps only, and no $L_K$ bound
follows: the two-element instance of Section~\ref{sec:model} caps the
stopped run at ratio $1/2$ against $L_2(1)=3/4$.
% M1 (J5 section 3): executed-steps scope made explicit; counterexample
% recomputed in exact rationals, results/M0_counterexamples.py part 1.
...
% ---------------------------------------------------------------------------
\subsection{The ceiling (Proposition~\ref{thm:ceiling})}\label{app:ceiling}
...
\paragraph{The unified per-instance guarantee.}
% J5H2: transcribed from results/J5_hardcore/J5_ceiling_proof.md (J5 audit,
% batch A).  The three displayed decompositions below are
% [VERIFIED-SYMBOLIC] (results/J5_hardcore/J5_hardcore_oracles.py); the
% general exchange, telescoping and minimax assembly is
% [HAND-PROOF-UNREVIEWED, source J5].
The following argument closes the case $K\le n\le2K$ and reproves
$n\ge2K$, giving the per-instance form of Proposition~\ref{thm:ceiling}.  Let
$S$ maximize $\tilde f$ over all sets of size exactly $K$ (the band makes
$\tilde f$ monotone, so completing a smaller predicted maximizer to size
$K$ loses nothing), let $O$ be an optimal $K$-set, and put
$a=|O\setminus S|$, $A=f(S)$, $B=f(O)$, $Z=f(S\cup O)$.

\emph{Step 1 (all exchanges, summed).}  For $s\in S$ write
$r_s=f(S)-f(S\setminus\{s\})$ and $R=\sum_{s\in S}r_s$; adding the elements
of $S$ in any order, each increment is at least the end deletion loss of
that element, so $0\le R\le A$.  For $b\in O\setminus S$ and every $s\in S$
the predicted optimality of $S$ gives
$\tilde d_b(S\setminus\{s\})\le\tilde d_s(S\setminus\{s\})$, and with the
two band sides and diminishing returns,
$d_b(S)\le d_b(S\setminus\{s\})\le\eta_u\,\tilde d_b(S\setminus\{s\})
\le\eta_u\,\tilde d_s(S\setminus\{s\})\le\eta\,r_s$.  Summing over all
$aK$ pairs $(b,s)$, with $D=\sum_{b\in O\setminus S}d_b(S)$,
\[
  KD\;\le\;\eta aR\;\le\;\eta aA,
  \qquad\text{hence}\qquad
  Z\;\le\;A+D\;\le\;\Bigl(1+\frac{\eta a}{K}\Bigr)A
\]
by submodularity along $S\to S\cup O$.  The deletion side ranges over all
$K$ elements of $S$, intersection included; the exchanges are not
restricted to $S\setminus O$.

\emph{Step 2 (two bands coupled on one union).}  With
$X=\tilde f(S\cup O)-\tilde f(S)$ and $Y=\tilde f(S\cup O)-\tilde f(O)$,
predicted optimality gives $X\le Y$, and telescoping both bands along the
two chains into the union gives $(Z-A)/\eta_u\le X\le Y\le\eta_o(Z-B)$,
hence $B\le(1-\tfrac1\eta)Z+\tfrac A\eta$.  This uses the values of the
single predicted set function on $S$, $O$ and $S\cup O$; the algorithm
need not have queried the union, and $\tilde f$ need not be submodular.

\emph{Step 3 (eliminate the union).}  Substituting Step 1 into Step 2 and
using $\eta\ge1$,
\[
  B\;\le\;\Bigl(1-\frac1\eta\Bigr)\Bigl(1+\frac{\eta a}{K}\Bigr)A
  +\frac A\eta
  \;=\;\Bigl(1+\frac{(\eta-1)a}{K}\Bigr)A ,
\]
which is the per-instance bound; $a\le\min\{K,n-K\}$ then gives
$C^{*}_{n,K}$.  With $\gamma=1-1/\eta$ and
$E=\gamma Z+A/\eta-B$, $H=A+D-Z$, $J=\eta aR-KD$, $T=A-R$, the whole
argument is one nonnegative combination:
\begin{equation}\label{eq:ceiling-slack}
  \Bigl(1+\frac{(\eta-1)a}{K}\Bigr)A-B
  \;=\;E+\gamma H+\frac{\gamma}{K}J+\frac{\gamma\eta a}{K}T ,
\end{equation}
an identity in $A,B,Z,D,R,a,K,\eta$, with
$E=\frac{\eta_o(Z-B)-Y}{\eta_o}+\frac{Y-X}{\eta_o}
+\frac{\eta_uX-(Z-A)}{\eta}$ and, per exchange,
$\eta\,r_s-d_b(S)$ decomposing into four nonnegative terms (upper band,
predicted exchange optimality, lower band, submodularity).  These
identities are checked symbolically; the earlier obstruction at
$f(S\cap O)$ never appears, because the argument routes through the union
and the full exchange sum.

\paragraph{The matching adversary for every $n>K$.}
Feed the algorithm $\tilde f(T)=b|T|$; its transcript and output are
fixed.  Complete the output to a $K$-set $S$ if necessary, and let $f$ be
modular with weights $b/\eta_o$ on $S$ and $\eta_ub$ off $S$.  With
$a_0=\min\{K,n-K\}$ an optimal set takes $a_0$ heavy and $K-a_0$ light
elements, so $f(S)/f(O)=K/(K+(\eta-1)a_0)=C^{*}_{n,K}(\eta)$, and a
smaller output is only worse.  Every set increment is a sum of weights, so
the single-element and all-pairs errors equal the prescribed split
exactly.  At $n=K$ the whole ground set is the output (ratio $1$); if both
error endpoints must be attained there, mix high and low weights on at
least two elements.

\paragraph{Scope.}
The lower-bound quantifier is: there exists an algorithm (exhaustive
search) that guarantees $C^{*}_{n,K}$ on every instance; no claim is made
for other algorithms, and one-swap local optimality of the output is not
enough for Step 1, which uses all $aK$ exchange comparisons.  Exhaustive
search evaluates $\binom nK$ predicted sets; nothing here produces a
polynomial-query algorithm or changes the scope of
Theorem~\ref{thm:hardness}.  The randomized value is genuinely different: at $n=3$, $K=2$, $\eta=3$ the
deterministic value is $1/2$, while the uniform random $2$-subset has
expected value at least $\tfrac23f(N)$ (sum nonincreasing expected
marginals along a random permutation); the randomized minimax value
remains open.

The slack identities of this subsection and $52$ full-lattice
algorithm-side LPs, together with $24$ exact rational modular adversary
instances ($37{,}056$ all-pairs gains), are checked by
\texttt{results/J5\_hardcore/J5\_hardcore\_oracles.py}; the general
assembly is hand-written (source: the J5 audit) and is tagged accordingly.
...
% J5H5: the former "GAP LEFT OPEN" comment is obsolete.  The monotonicity
% of rho_K in K and the 1/K expansion were closed on night 6 (H-B) and are
% migrated into this subsection below, as the J5 audit required; ledger T9
% carries the unified status.

\paragraph{The $1/K$ expansion.}
Fix $\eta\ge1$ and write $m=\lfloor\eta\rfloor$.  Expanding the active
branch $V_{K-m}$ at $K\to\infty$ (substitute $K=1/\varepsilon$ and expand
the exponential of $(K-m)\log q$) gives
\[
  \rho_K(\eta)\;=\;1-e^{-1/\eta}
  +\frac{e^{-1/\eta}(2\eta-1)}{2\eta^{2}}\cdot\frac1K
  +O_\eta\Bigl(\frac1{K^{2}}\Bigr),
\]
and the $1/K$ coefficient $c(\eta)$ does not depend on $m$: at integer
$\eta$ the two adjacent active branches give the same coefficient.  The
same expansion for $L_K$ and $U_K$ yields the companion coefficients
$c_L(\eta)=e^{-1/\eta}/(2\eta^{2})$ and $c_U=c$, so at fixed $\eta$
\[
  \rho_K-L_K=\frac{e^{-1/\eta}(\eta-1)}{\eta^{2}K}+O_\eta\Bigl(\frac1{K^{2}}\Bigr),
  \qquad
  U_K-\rho_K=O_\eta\Bigl(\frac1{K^{2}}\Bigr),
\]
the second difference being the $c'(\eta)/K^{2}$ band of
Remark~\ref{rem:greedybudget}.
% Status: the coefficients are [VERIFIED-SYMBOLIC], conditional on
% thm:exact through the closed form (results/H_B_asymptotic.py section 2;
% independently recomputed by results/J5_hardcore/J5_hardcore_oracles.py);
% the Taylor-remainder assembly is [HAND-PROOF-UNREVIEWED]; uniformity of
% the remainder in eta is not claimed (ledger T9).

\paragraph{Monotonicity of $\rho_K$ in $K$.}
For $K\le\lfloor\eta\rfloor$, $\rho_K(\eta)=1/\eta$ (the plateau of
Theorem~\ref{thm:exact}), and the first step off the plateau is a strict
drop by $(K-\eta)/(K\eta k_1)>0$ at $K=\lfloor\eta\rfloor+1>\eta$.  For
$K\ge\lfloor\eta\rfloor$ write $\rho_K=1-P(K)$ with
$P=q^{K-m}\bigl(1-\tfrac m{K\eta}\bigr)$ and $x=(K-1)\eta+1$; then
\[
  \frac{\partial\log P}{\partial K}
  =\log\Bigl(1-\frac1x\Bigr)+\frac{K-m}{(K-1)x}+\frac{m}{K(K\eta-m)} .
\]
Bounding $-\log(1-1/x)\le1/x+1/(2x^{2})+1/(3x^{2}(x-1))$ for $x>1$
(compare the tail of the logarithm series termwise with $1/i\le1/3$ for
$i\ge3$) and clearing denominators after the substitution $s=t/(1+t)$,
which covers the whole domain $m\ge1$, $K\ge m+1$, the difference becomes
a ratio of two polynomials with only nonnegative coefficients ($84$
numerator terms with constant term $14$; $165$ denominator terms with
constant term $48$), so $\partial\log P/\partial K>0$, $P$ increases and
$\rho_K$ strictly decreases in $K$ past the plateau.  Integer $\eta$
corresponds to the endpoint $s=0$ and is included.
% Status: the derivative identity, the tail bound and the two
% nonnegative-coefficient certificates are [VERIFIED-SYMBOLIC], conditional
% on thm:exact (results/H_B_asymptotic.py sections 4-5, exact rational
% differences K = 2..400 at 7 eta values; independently rerun by the J5
% hardcore oracle with the same 84/165-term counts); passing from the
% continuous derivative on each branch to the discrete differences,
% including the branch switches at integer eta, is the assembled hand
% argument [HAND-PROOF-UNREVIEWED].  The full coefficient tables live in
% the H-B and J5 JSONs and are deliberately not printed here.
...
% ---------------------------------------------------------------------------
\subsection{Exact optimality at the greedy budget
  (Theorem~\ref{thm:linear-exact})}
\label{app:greedybudget}
% Q4 (ledger card T10c).  This subsection replaces the former one-sided
% U_K ceiling (ledger T10b) by the exact value rho_K, via a new hard
% family.  Status decomposition:
%  - legality of the family (monotone, submodular, band on all four edge
%    classes, normalization, exact calibration at both split endpoints,
%    the rigid trajectory, value rho_K at every n >= 2K): the 13 phase and
%    edge identities and the 34 branch inequalities behind (i)-(iii) below
%    are [VERIFIED-SYMBOLIC] (results/Q4_gpt_check.py and
%    results/Q4_symbolic_ineq.py), and the whole grid is checked in exact
%    rationals on 111 + 159 parameter configurations, K up to 13, both
%    tied branches at integer eta, both extreme splits
%    (results/Q4_indep_check.py, results/Q4_gpt_check.py).
%  - the counting chain below is [VERIFIED-SYMBOLIC]
%    (results/L1_table.py section 1), reused from the former corollary.
%  - the count-grid-to-set-function chaining, the canonical-transcript
%    induction (that of app:hardness) and the two averagings are
%    [HAND-PROOF-UNREVIEWED] (source: the Q4 external parallel audit),
%    and per the ledger this tag is NOT to be upgraded here.

\paragraph{The double-residual family.}
Fix $K\ge2$, $\eta>1$ and a split $\eta_u,\eta_o\ge1$ with
$\eta_u\eta_o=\eta$.  Set $k_1=(K-1)\eta+1$, $q=(K-1)\eta/k_1$,
$j=\max\{0,K-\lfloor\eta\rfloor\}$, $Q=q^{j}$, $\delta=Q/(K\eta)$,
$C=k_1/K$ and $[z]_+=\max\{z,0\}$, and define two residual sequences on
integers $x\ge0$:
\[
  r_x=\begin{cases}q^{x},&0\le x\le j,\\[2pt]
    \bigl[Q-(x-j)\delta\bigr]_+,&x\ge j,\end{cases}
  \qquad
  h_x=\begin{cases}\tfrac{K-1}{K}\,q^{x},&0\le x\le j,\\[2pt]
    \bigl[\tfrac{K-1}{K}Q-(x-j)\delta\bigr]_+,&x\ge j,\end{cases}
\]
the two branches agreeing at $x=j$.  On a ground set $N=B\sqcup O$ with
$|O|=K$ and $|B|=n-K$, write $x=|S\cap B|$, $y=|S\cap O|$ and
$c_y=(K-y)/(K-1)$, and define the objective and the scaled predictor
$H=\eta_u\tilde f$ by
\[
  f(S)=F(x,y)=\begin{cases}1-r_x,&y=0,\\1-c_y\,h_x,&1\le y\le K,\end{cases}
  \qquad
  H(x,y)=\begin{cases}C-r_x-(\eta-1)h_x,&y=0,\\
    C-\eta\,c_y\,h_x,&1\le y\le K.\end{cases}
\]
The formulas do not involve $n$: every $n\ge2K$ restricts the same table
to its available count grid, and no saturation threshold needs to fit
inside it.

\paragraph{Legality.}
Set $p_x=r_x-r_{x+1}$ and $s_x=h_x-h_{x+1}$.  Three groups of
one-dimensional facts about the residuals carry the whole verification:
(i)~$0\le s_x\le p_x$ and both sequences are nonincreasing, the junction
satisfying $p_{j-1}=Q/((K-1)\eta)\ge\delta=s_{j-1}$;
(ii)~$r_x-h_x\ge h_x/(K-1)\ge0$, with equality in the first inequality on
$x\le j$; (iii)~the no-truncation window
$(K-j)\delta\le Q/K\le(K-1)Q/K$, so neither residual is truncated before
$x=K$.  Each is verified branch by branch (geometric phase, junction,
linear phase, truncation crossings, and the $j=0$ regime $\eta\ge K$) in
\texttt{results/Q4\_symbolic\_ineq.py}.  They give everything at once.
The first differences of $F$ are
\[
  \Delta_xF(x,0)=p_x,\quad
  \Delta_xF(x,y)=c_y s_x,\quad
  \Delta_yF(x,0)=r_x-h_x,\quad
  \Delta_yF(x,y)=\frac{h_x}{K-1}\ \ (1\le y<K),
\]
all nonnegative by (i)--(ii); each is nonincreasing in $x$ and in $y$,
which is submodularity on the count grid, and chaining single-element
additions transfers monotonicity and diminishing returns to the set
function $f$.  Moreover $F(0,0)=0$, $F(0,K)=1$ and $0\le F\le1$ on the
entire grid, so $O$ is optimal under the cardinality constraint and
$f(O)=1$.  On the predictor side the four edge classes are
\[
  \begin{array}{l@{\qquad}l@{\qquad}l}
    \text{$B$-edge, }y=0: & \Delta F=p_x, & \Delta H=p_x+(\eta-1)s_x,\\
    \text{$B$-edge, }1\le y\le K: & \Delta F=c_y s_x, & \Delta H=\eta\,c_y s_x,\\
    \text{$O$-edge, }y=0: & \Delta F=r_x-h_x, & \Delta H=r_x-h_x,\\
    \text{$O$-edge, }1\le y<K: & \Delta F=h_x/(K-1), & \Delta H=\eta\,h_x/(K-1),
  \end{array}
\]
and $0\le s_x\le p_x$ gives $\Delta F\le\Delta H\le\eta\,\Delta F$ in the
first row; the other rows satisfy the same inequalities with ratio exactly
$1$ or exactly $\eta$.  Hence on every edge
$\Delta F/\eta_u\le\Delta\tilde f\le\eta_o\,\Delta F$ for
$\tilde f=H/\eta_u$; zero-gain edges have $\Delta H=0$, and
$H(0,0)=C-1-(\eta-1)(K-1)/K=0$ gives $\tilde f(\varnothing)=0$.  Both
split endpoints are attained on positive-gain edges already at $x=0$ (the
first $O$-edge has ratio $1/\eta_u$ in $\tilde f$-form, the second
$\eta_o$), so the smallest admissible error factors of
Definition~\ref{def:eta} are exactly $(\eta_u,\eta_o)$.

\paragraph{Small-set indistinguishability and the rigid trajectory.}
Define
\[
  \widehat H_s=\begin{cases}C\,(1-q^{s}),&0\le s\le j,\\
    C\,(1-Q)+\eta\,(s-j)\,\delta,&j\le s\le K.\end{cases}
\]
Using $Cq=\eta(K-1)/K$ one checks $H(x,0)=\widehat H_{x}$ for $0\le x\le K$
and $H(x,1)=\widehat H_{x+1}$ for $x+1\le K$, so
\[
  \tilde f(S)=\widehat H_{|S|}/\eta_u
  \qquad\text{whenever } |S|\le K \text{ and } |S\cap O|\le1:
\]
on every set the transcript below can meet, the predictor depends on
$|S|$ alone.  This is where the family departs from the size-based
constructions of Appendix~\ref{app:hardness-anysize}: on larger sets the
predictor genuinely leaks (at $K=3$, $\eta=3/2$ the two $6$-sets
$(x,y)=(6,0)$ and $(5,1)$ receive $61/48$ and $4/3$), and this leak is
what frees the small-set values from the long-range constraints that
force a strict excess above $\rho_K$ in the globally size-based regime
(\texttt{results/Q3\_failure\_point.md}; with $O$-independence imposed
only on $|S|\le K$, the count-grid LP attains $\rho_K$ at every tested
$n$, \texttt{results/Q4\_smallset\_lp.py}).
At every state $(t,0)$, $t<K$, the predicted marginals of all remaining
elements tie at $q^{\min(t,j)}/(K\eta_u)$, so adversarial tie-breaking
keeps greedy inside $B$ for all $K$ steps; the true selected gains are
$d_t=q^{t}/k_1$ for $t<j$ and $q^{j}/(K\eta)$ for $j\le t<K$, while every
remaining optimal element keeps true marginal $q^{\min(t,j)}/K$: the
trajectory prescribed by Proposition~\ref{prop:rigidity} is realized
exactly.

\paragraph{The value.}
By (iii) no truncation occurs before $x=K$, so for every $K$-set
$T\subseteq B$,
\[
  \frac{f(T)}{f(O)}=F(K,0)=1-Q+(K-j)\,\delta=V_{j}(\eta),
\]
and the adjacent-branch identity
$V_{j+1}(\eta)-V_{j}(\eta)=q^{j}(\eta-K+j)/(K\eta k_1)$ shows that
$j=\max\{0,K-\lfloor\eta\rfloor\}$ minimizes over branches, with ties
exactly at integer $\eta$.  Hence $F(K,0)=\rho_K(\eta)$ for every
$n\ge2K$: the value is exact at each $n$, and no limit is taken.
...
balanced and misses $T_0$.

\paragraph{Transcript induction and the two directions.}
Run the algorithm against the canonical oracle
$S\mapsto\widehat H_{|S|}/\eta_u$ for $|S|\le K$; this fixes queries
$S_1,\dots,S_Q$ and output $T_0$ independently of $O$.  Fix a $K$-set $O$
outside the bad event above.  The transcript induction of
Appendix~\ref{app:hardness}, with balanced meaning $|S_i\cap O|\le1$,
forces the actual run on the family with this $O$ to reproduce the
canonical one: identical previous answers determine the same next query,
and the small-set identity above supplies the same answer.  The actual
output is therefore $T_0$, disjoint from $O$ and of size at most $K$, so
monotonicity and the value paragraph give
$f(T_0)/f(O^{\ast})\le F(K,0)=\rho_K(\eta)$, with error exactly $\eta$
and any prescribed split realized directly by $\tilde f=H/\eta_u$.  For
the matching direction, predictive greedy spends
$\sum_{t=0}^{K-1}(n-t)=nK-K(K-1)/2\le nK$ queries, all of size at most
$K$, by retaining previously queried values, and achieves
$\rho_K(\eta)$ on every instance with error at most $\eta$
(Theorem~\ref{thm:exact}); together these give the sup--inf identity of
Theorem~\ref{thm:linear-exact}.
...
\paragraph{Randomized algorithms.}
The averaging of Appendix~\ref{app:hardness}, applied with the union
bound above, yields for every randomized algorithm of the class a single
instance with
$\mathbb E[f(T)/f(O^{\ast})]\le\rho_K(\eta)+K/n+K^{5}/(2n)$: fixing the
seed and its canonical transcript, on the success event
$f(T_0)\le\rho_K(\eta)+|T_0\cap O|/K$, because each element of $O$ has
singleton value $F(0,1)=1/K$ and submodularity caps its marginal by its
singleton value, while on the failure event $f\le1$; averaging over $O$
bounds $\mathbb E|T_0\cap O|/K$ by $K/n$, and averaging over the seed and
then selecting one $O$ no worse than the average gives the display.  The
theorem states the slightly larger
$\varepsilon_n=K^{2}/n+K^{5}/(2n)$, matching the deterministic output
count.  The quantifier order is that of Theorem~\ref{thm:hardness}: the
instance may depend on the algorithm, and the expectation is over the
algorithm's randomness only.  The additive term vanishes only in the
joint limit $n/K^{5}\to\infty$; at fixed $n$ it does not, and no exact
finite-$n$ randomized statement is claimed.
% M2.6 (J5) carried over: joint-limit condition explicit; Q4 (T10c): the
% value inside the expectation upgraded from U_K to rho_K; the
% F <= rho_K + y/K step is part of the [HAND-PROOF-UNREVIEWED] assembly.

\paragraph{The improvement over the former ceiling.}
For the improvement statement of Remark~\ref{rem:greedybudget}, expanding
both closed forms at fixed $\eta$ in $1/K$ (with $m=\lfloor\eta\rfloor$
and $\rho_K=V_{K-m}$ on the active branch) gives
\[
  U_K(\eta)-\rho_K(\eta)
...
with one Richardson step); the two curves are tabulated in
\texttt{results/L1\_table.csv}.  All of this is conditional on
Theorem~\ref{thm:exact} through the closed form of $\rho_K$.

% ---------------------------------------------------------------------------
\subsection{Partial enumeration under predictions: an exact rational
counterexample}\label{app:pe}
% J5H3 (ledger T11; source: results/J5_hardcore/J5_variant_certificates.md
% sections 4-5, oracle results/J5_hardcore/J5_hardcore_oracles.py).
In the exact-oracle setting, partial enumeration improves the finite-$K$
greedy constant \citep[Section~7, Theorem~7.1]{nemhauser1978analysis}.
Under predictions this can invert.  Let $\mathrm{PE}_1$ run predictive
greedy from every singleton start (adversarial ties inside each run) and
return the completion with the largest predicted value, final ties again
adversarial.  On the following seven-element instance with $K=3$ and
$(\eta_u,\eta_o)=(2,1)$ it is strictly worse than plain predictive greedy
and strictly below $\rho_3(2)$.

For $T\subseteq\{0,\dots,6\}$ write $x=\mathbf 1_{\{0\in T\}}$,
$b=|T\cap\{1,2\}|$, $y=|T\cap\{3,4,5\}|$, $z=\mathbf 1_{\{6\in T\}}$, and
\[
  18f(T)=2x+3b+6y-3b\,\mathbf 1_{\{y=3\}}+z\,\Delta_f(b,y),
  \qquad
  18\tilde f(T)=2x+3b+3y+3(1-b)\mathbf 1_{\{y=3\}}+z\,\Delta_g(b,y),
\]
with the two $3\times4$ correction tables
\[
  \Delta_f=\begin{pmatrix}5&4&2&0\\5&3&1&0\\4&2&0&0\end{pmatrix},
  \qquad
  \Delta_g=\begin{pmatrix}3&3&2&0\\3&2&1&0\\2&1&0&0\end{pmatrix}
  \qquad(\text{rows }b=0,1,2,\ \text{columns }y=0,\dots,3).
\]
All $128$ subset values, all $448$ edges ($0\le d/2\le\tilde d\le d$, both
band endpoints attained with positive gains), and all $672$ true
submodularity squares are verified in exact rationals; $O=\{3,4,5\}$ has
$f(O)=1$ and every $3$-set value is at most $1$.  One legal adversarial
run per start gives the endpoints
$(0{:}\,\{0,1,2\})$, $(1{:}\,\{1,6,0\})$, $(2{:}\,\{2,6,0\})$,
$(v{:}\,\{v,6,0\})$ for $v\in\{3,4,5\}$, $(6{:}\,\{6,1,0\})$, all with
predicted value $4/9$, so the final adversarial tie may select
$\{0,1,2\}$ with true value $4/9$; enumerating all legal trajectories
shows plain greedy's worst value on this instance is $5/9$.  Hence
\[
  \mathrm{PE}_1\;=\;\frac49\;<\;\frac7{15}\;=\;\rho_3(2)\;<\;\frac59 ,
\]
and the worst-case guarantee of $\mathrm{PE}_1$ at $(K,\eta)=(3,2)$ is
strictly below predictive greedy's.  Padding with zero elements extends
the counterexample to every $n\ge7$: old starts keep their runs, and a new
zero start first picks a predicted argmax $b$ of the original instance and
then follows a prefix of $b$'s completion run, whose predicted value
monotonicity keeps below the winning endpoint.
% Status: the instance and every displayed count are [VERIFIED-LP, exact
% rational exhaustion] (J5 hardcore oracle, rerun this session, exit 0);
% the padding extension is [HAND-PROOF-UNREVIEWED, source J5], with n=8,9
% checked exhaustively.  NOT claimed: a general-K analytic family; a
% uniform continuum statement in eta (the discrete scans support grid
% points only; the former "eta >= 1.75 as well" reading is retracted,
% ledger T11); that the losing endpoint must forfeit a full eta factor.

% ---------------------------------------------------------------------------
\subsection{Queries of arbitrary size
  (Proposition~\ref{thm:linear-anysize})}
\label{app:hardness-anysize}
% J6/J7 day (ledger card T10d; source J7, results/J7/linear_anysize.md).
% This subsection replaces the former candidate bound (J4 demotion, ledger
% T12) by a theorem: the night-4 truncation rule is corrected (the old
% m* = ceil(K eta) - 1 is refuted, see the closing note) and the family's
% legality is proved at general parameters.  Status decomposition:
%  - the Psi-rule structure, every displayed identity, and the count-grid
%    legality reductions: [VERIFIED-SYMBOLIC], results/J7_symbolic.py
%    (148 items, 0 failed; hand-proof residue: two classical elementary
%    inequalities, (1+1/u)^{u+1} > e and m(nu^l-1) <= l(nu^m-1), each
%    backed by exact sweeps), plus 99 exact-rational full-grid
%    configurations (results/J7_grid_check.py) and an exact finite oracle
%    for the exponential bound (results/J7_bound18_check.py, e replaced
%    by a rational upper bound so the decision is exact);
%  - the lift from the count grid to set functions, the leakage bound,
%    the canonical-transcript induction and the averaging:
%    [HAND-PROOF-UNREVIEWED], source J7;
%  - the lower bound of the theorem is Theorem~\ref{thm:exact}.
% Finite-n caveat (must-not-claim, ledger T10d): at n < K + t* reverse
% greedy provably reads O off the leaking large sets and reaches ratio
% exactly 1 (results/J7_grid_check.py, adversarial simulation), so the
% n -> infinity quantifier of the theorem cannot be dropped.
% NOTATION: as before, the window constant is written t* to avoid the
% output set T of Theorem \ref{thm:hardness}.

Theorem~\ref{thm:hardness} restricts every query to at most $K$ elements,
and Theorem~\ref{thm:linear-exact} inherits that restriction.  Removing it
costs an additive term exponentially small in $K$.  Fix $K\ge3$ and
$\eta>1$, keep $k_1=\eta(K-1)+1$, $q=1-1/k_1$, $\nu=\eta/(\eta-1)$, and
define
\[
  \Psi(t)=(K\eta-t-1)\,\nu^{t}-K(\eta-1),
  \qquad
  m=\min\{z\in\mathbb Z_{\ge1}:\Psi(z)\le0\}.
\]
The function $\nu^{-t}\Psi(t)$ is strictly concave, equals $K-1>0$ at
$t=0$ and tends to $-\infty$, so $m$ is well defined; since
$\nu^{\eta(K-1)}=(\nu^{\eta})^{K-1}>e^{K-1}>K$ (using
$(1+1/u)^{u+1}>e$ for $u=\eta-1>0$) and $\Psi(K\eta-1)=-K(\eta-1)<0$,
\[
  \eta(K-1)\;<\;m\;<\;K\eta.
\]
Set $B_m=\eta(\nu^{m}-1)-m>0$ and $d=(\nu^{m}/K-1)/B_m$.  The two
identities
\[
  d-\frac{1}{m+1}=\frac{-\Psi(m)}{K(m+1)B_m}\ge0,
  \qquad
  \frac1m-d=\frac{\nu\,\Psi(m-1)}{K\,m\,B_m}\ge0
\]
pin $d$ into $[1/(m+1),\,1/m]$, and
$d-1/(K\eta)=\bigl(m-\eta(K-1)\bigr)/(K\eta B_m)>0$.  With
$j=\max\{0,\min\{K-1,\,K+1-\lceil\eta\rceil\}\}$, $Q=q^{j}$, $D=Qd$ and
$t^{\ast}=j+m$, define two sequences on integers $x\ge0$,
\[
  r_x=\begin{cases}q^{x},&x\le j,\\ Q-(x-j)D,&j<x\le t^{\ast},\\
    0,&x>t^{\ast},\end{cases}
  \qquad
  g_x=\begin{cases}q^{x}/K,&x\le j,\\
    ...
    0,&x>t^{\ast},\end{cases}
\]
$a_x=r_x-g_x$, and, with $x=|S\cap B|$, $y=|S\cap O|$,
$c_y=(K-y)/(K-1)$ and $C=k_1/K$, the instance
\[
  f=F(x,y)=\begin{cases}1-r_x,&y=0,\\1-c_y\,a_x,&y\ge1,\end{cases}
  \qquad
  \eta_u\tilde f=H(x,y)=\begin{cases}C-r_x-(\eta-1)a_x,&y=0,\\
    C-\eta\,c_y\,a_x,&y\ge1.\end{cases}
\]

\paragraph{Legality.}
The choice of $d$ makes the closing step legal:
$r_{t^{\ast}}=g_{t^{\ast}}$ exactly, with
$0\le r_{t^{\ast}}\le D$ equivalent to $d\in[1/(m+1),1/m]$.  On the
plateau $g_{x+1}=\nu(g_x-D)$, and on every segment
\[
  a_x-a_{x+1}=\frac{g_{x+1}}{\eta}.
\]
From these facts: $g$ and $a$ are nonnegative and nonincreasing; the
steps of $r$ are nonincreasing (geometric decrease, then the plateau
constant $D$, then the final step $r_{t^{\ast}}\le D$, with the junction
$p_{j-1}=Q/((K-1)\eta)\ge D$ because $d\le1/m<1/(\eta(K-1))$); the steps
of $r$ dominate those of $a$; and $Kg_x\ge r_x$ throughout, because on
the plateau $Kg_{j+\ell}-r_{j+\ell}$ is concave in $\ell$, zero at
$\ell=0$ and equal to $(K-1)\,r_{t^{\ast}}\ge0$ at $\ell=m$ (equality
holds on the geometric phase and both sides vanish past $t^{\ast}$).
These one-dimensional facts yield every monotonicity and
diminishing-returns inequality of $F$ on the count grid, and the same
four-edge-class table as in Appendix~\ref{app:greedybudget} yields
$\Delta F\le\Delta H\le\eta\,\Delta F$ on every edge, with both split
endpoints attained on positive-gain edges at $x=0$; moreover
$F(0,0)=H(0,0)=0$, $F(0,K)=1$ and $0\le F\le1$ on the whole grid, so $O$
is optimal with $f(O)=1$ and the smallest admissible error factors are
exactly $(\eta_u,\eta_o)$.

\paragraph{Arbitrary query sizes.}
The identity $a_x-a_{x+1}=g_{x+1}/\eta$ is equivalent to
$H(x+1,0)=H(x,1)$ for \emph{every} $x\ge0$: the predictor depends only on
$|S|$ on the whole region $|S\cap O|\le1$, at every size, and for
$x>t^{\ast}$ both functions saturate ($F=1$, $H=C$).  A query can
therefore distinguish the hidden $O$ only if $|S\cap O|\ge2$ and
$|S|\le t^{\ast}+K$.  For a uniformly random $K$-set $O$, one such query
leaks with probability at most $K^{2}(t^{\ast}+K)^{2}/(2n^{2})$, so $cnK$
queries leak with total probability at most
$cK^{3}(t^{\ast}+K)^{2}/(2n)\to0$; the canonical-transcript induction and
the averaging are those of Appendix~\ref{app:hardness}.

\paragraph{Value and the exponential gap.}
Every $K$-set disjoint from $O$ has value
$W_K(\eta)=1-Q+(K-j)\,Q\,d$, and
\[
  W_K(\eta)-\rho_K(\eta)
  =\frac{(K-j)\,Q\,\bigl(m-\eta(K-1)\bigr)}
        {K\eta\,\bigl(\eta(\nu^{m}-1)-m\bigr)}
  \;\in\;\Bigl(0,\ \frac{1}{K\,(e^{K-1}-K-1)}\Bigr)
\]
for $K\ge3$, using $K-j\le\eta$, $Q\le1$, $m-\eta(K-1)<\eta$ and
$\nu^{m}>e^{K-1}$.  This proves Proposition~\ref{thm:linear-anysize}, the
lower bound being Theorem~\ref{thm:exact}.  At $K=3$, $\eta=3/2$: $m=4$,
$d=13/58$, and $W_3(3/2)=523/928=9/16+1/928$ against
$\rho_3(3/2)=9/16$.

\paragraph{Correction of the night-4 truncation rule.}
The night-4 draft (\texttt{results/F3\_hardness\_full.tex}) uses
$m=\lceil K\eta\rceil-1$, which is not admissible in general: at $K=3$,
$\eta=667/500$ it gives $m=4$ and
$r_{t^{\ast}}=-151089222203006/81950355825200625<0$, a negative
$O$-marginal, while the $\Psi$-rule gives $m=3$
(\texttt{results/J7\_fragment\_checks.py}, exact rationals; the same
corner-point failure class, small $\operatorname{frac}(K\eta)$, as the
four counterexamples of \texttt{results/Q2\_symbolic.md}).  On $303$
exact-rational test points the $\Psi$-rule agrees with the direct argmax
of the excess slope, and two identities explain the agreement
structurally (\texttt{results/J7\_symbolic.py}, section S12).
\paragraph{Beyond the linear budget.}
The night-4 draft's superlinear reading, budgets up to
$Q\le n^{2}/\bigl(2K^{2}(t^{\ast}+K)^{2}\bigr)$, remains a candidate
bound of the supplementary material and not a theorem of this paper
(ledger T12; \texttt{results/F3\_hardness\_full.tex}).  Since
$t^{\ast}+K\le(2+\eta)K+1$, that budget is
$\Omega\bigl(n^{2}/((2+\eta)^{2}K^{4})\bigr)$: for fixed $K$ and $\eta$ a
polynomial budget of degree approaching $2$, and not every polynomial;
a uniform reading such as $n^{2-o(1)}$ would additionally require
restricting how $K$ and $\eta$ grow with $n$.  Within the checked
configurations the exponent $2$ is a property of the family rather than
of the analysis: it is the exponent in
$\Pr[|S\cap O|\ge2]\le(Ks/n)^{2}/2$ for a query of size $s$, raising the
tolerance to $|S\cap O|\le\tau$ with $\tau\ge2$ is infeasible by explicit
two-constraint certificates at all $48$ tested configurations, and
requiring the size-only property on the whole two-sided band
$\bigl||S\cap O|-K|S|/n\bigr|\le\tau$ is feasible exactly when $\tau=1$
and $n>K(t^{\ast}+1)$.  Numerically the excess decays fast: at $\eta=2$
...
$W_K(\eta)\to1-e^{-1/\eta}$.
% Decay row recomputed under the Psi rule this session (identical to the
% old figures at these points); chain and limit: night-4 enumerations
% results/N4_check.py, N4_symbolic.py, plus this session's
% J7_bound18_check.py.
