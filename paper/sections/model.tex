...
% M1 (J5 audit, sections transcribed from TASKS8): the n < K, OPT = 0 and
% empty-trajectory conventions are stated once, here.
...
Only the product $\eta=\eta_u\eta_o$ matters for worst-case statements, in
the following class sense.
\begin{lemma}[Scaling]\label{lem:scaling}
Write $\mathcal F(\eta_u,\eta_o)$ for the set of predictors satisfying
Definition~\ref{def:eta} for a given monotone submodular $f$.  For every
$c\in[1/\eta_o,\ \eta_u]$ the map $\tilde f\mapsto c\tilde f$ is a bijection
from $\mathcal F(\eta_u,\eta_o)$ onto $\mathcal F(\eta_u/c,\ c\,\eta_o)$,
whose factors are again at least $1$ and have the same product.  The map
preserves $f$ and every comparison of predicted values, so each run of a
comparison-based algorithm, predictive greedy in particular, and hence its
worst-case ratio over the class, depends on $(\eta_u,\eta_o)$ only through
$\eta$.
\end{lemma}
% M1 (J5 "Lemma 0'" item, spec transcribed from TASKS8): stated for band
% CLASSES, not for the smallest admissible factors of an individual
% predictor (those move under scaling).  Status: three lines of algebra,
% [HAND-PROOF-UNREVIEWED]; the band transformation and the endpoints of the
% c-interval are checked symbolically in results/M1_checks.py.  Algorithms
% that read predicted VALUES rather than comparisons are not covered by the
% run-invariance clause; hardness statements handle their splits by
% explicit rescaling (Appendix \ref{app:hardness}).
...
% M1 (J5 section 3 counterexample, independently recomputed in exact
% rationals: results/M0_counterexamples.py part 1).  The fixed-K semantics
% was already the stated one; this sentence makes it explicit and prices
% the stopping variant.  Two run-dependent error
measures refine $\eta$:
