...
The ceiling of Proposition~\ref{thm:ceiling} places no restriction on the query
...
Proposition~\ref{thm:ceiling} (at $\eta=2$, $c=2$, $K=8$ it lies above $1/2$),
...
is coarser than the budget predictive greedy actually spends.  Re-running
the counting argument of Theorem~\ref{thm:hardness} at that exact budget,
against a hard family built so that its predictor depends only on $|S|$ on
precisely the sets such an algorithm can query, pins down the optimum of
the whole budget class exactly, and the value is greedy's own worst case
$\rho_K(\eta)$: no algorithm with greedy's query resources beats greedy,
at any error level and any $K$.

\begin{theorem}[Exact optimality within the greedy query budget]
\label{thm:linear-exact}\label{cor:greedybudget}
% Q4 (ledger card T10c): upgraded from the earlier one-sided corollary
% "An upper bound within the greedy query budget" (ledger T10b); the U_K
% ceiling is replaced by the exact value rho_K.
...
\end{theorem}
% Proof: Appendix \ref{app:greedybudget} -- a new hard family (the
% double-residual truncated family, ledger card T10c) whose predictor is a
% function of |S| exactly on the sets the transcript can query (|S| <= K,
% |S cap O| <= 1), with the counting chain of the former corollary (ledger
% T10b) reused verbatim.  Status decomposition: the legality of the family
% (monotone, submodular, band on all four edge classes, normalization,
% exact calibration at both split endpoints, the rigid trajectory of
% prop:rigidity, value rho_K at every n >= 2K) is [VERIFIED-SYMBOLIC]
% (13 identities: results/Q4_gpt_check.py; 34 branch inequalities:
% results/Q4_symbolic_ineq.py) plus exact-rational full-grid batteries
% (111 + 159 configurations, results/Q4_indep_check.py, Q4_gpt_check.py);
% the counting chain is [VERIFIED-SYMBOLIC] (results/L1_table.py); the
% count-grid-to-set-function chaining, the canonical-transcript induction
% and the two averagings are [HAND-PROOF-UNREVIEWED] (source: the Q4
% external parallel audit).
% Vacuity check: "deterministic" -- the randomized sentence differs by
% eps_n and is asymptotic only.  "at most nK queries" -- without a budget,
% Theorem \ref{thm:ceiling} alone applies and its constant 1/eta is
% different.  "each on a set of size at most K" -- essential: the family's
% predictor genuinely leaks on larger sets (at K=3, eta=3/2 the two 6-sets
% (x,y)=(6,0) and (5,1) get different predictions, 61/48 vs 4/3), so the
% any-size regime (Appendix \ref{app:hardness-anysize}) remains open.
% "n >= 4K^5" -- explicit sufficient condition for failure < 1/2; cannot
% be dropped wholesale (n=4, K=2 exhaustive-search counterexample in the
% remark below); the same arithmetic covers any budget Q <= n^2/(4K^4).
% "exactly optimal" -- both directions quantified above; the instance in
% (i) depends on the algorithm and the split.
% "output a set of size at most K" -- part of the query model, as in
% thm:hardness.

\begin{remark}\label{rem:greedybudget}
An earlier version of Theorem~\ref{thm:linear-exact} (ledger T10b) only
capped $\mathcal A_{\mathrm{lin}}$ at $\min\{U_K(\eta),1/\eta\}$, leaving
the interval $[\rho_K(\eta),\,\min\{U_K(\eta),1/\eta\}]$ for the class
optimum; the theorem closes that interval from within, at every
$n\ge4K^{5}$.  Two calibration points survive unchanged.  For $\eta\ge K$
the old interval was already degenerate ($\rho_K(\eta)=1/\eta$ by
Theorem~\ref{thm:exact}).  For $1<\eta<K$, at fixed $\eta$ as
$K\to\infty$, the amount by which the exact value improves on the old
ceiling is
\[
  ...
  \;>\;0,
\]
so on the scale of the constants involved the closure settles a band of
width $O(1/K^{2})$: small, but strictly positive at every $\eta\in(1,K)$.
...
survives.
At $\eta=1$ the same conclusion holds by the earlier route: the
calibration reads $\bar\theta=1$, where the family of
Appendix~\ref{app:hardness} degenerates to $\tilde f=f$ (error exactly
$1$), and the constant $U_K(1)=L_K(1)=\rho_K(1)$, so for $n\ge4K^{5}$
predictive greedy is exactly optimal within $\mathcal A_{\mathrm{lin}}$
at $\eta=1$, continuously matching Theorem~\ref{thm:linear-exact} as
$\eta\downarrow1$.  The case
...
$f(O^{\ast})/\eta$ on every instance, matching Proposition~\ref{thm:ceiling},
...
% Status: the lower end of the settled band is the >= direction of
% thm:exact (every feasible run is a point of the reduced LP, so its ratio
% is at least the LP value rho_K; instances with error below eta are
% feasible for the eta-LP).
...
% Q4 (ledger T10c): the former open problem -- greedy optimal in A_lin at
% finite K and 1 < eta < K -- is CLOSED by thm:linear-exact for
% n >= 4K^5; the any-size-query regime is thm:linear-anysize below
% (ledger T10d), exact up to an exponentially small additive term; what
% remains open is that term, the randomized class at finite n, and budget
% scales beyond O(nK).  The H-F evidence (PE_1 falls below rho_3 for
% n >= 7 at eta in {2, 2.5}) stays consistent finite-parameter support.

\begin{proposition}[Arbitrary query sizes at linear budget]
\label{thm:linear-anysize}
% J6/J7 day (ledger card T10d; source J7).  Proof:
% Appendix~\ref{app:hardness-anysize}.
...
\end{proposition}
% Status (ledger T10d): count-grid legality of the hard family, the
% corrected truncation rule (Psi criterion) with its location bounds, and
% the value/gap arithmetic are [VERIFIED-SYMBOLIC]
% (results/J7_symbolic.py, 148 items; residue: two classical elementary
% inequalities, each with exact sweeps), with 99 exact-rational
% configurations (results/J7_grid_check.py) and an exact finite oracle for
% the exponential bound (results/J7_bound18_check.py); the set-function
% lift, the leakage bound, the transcript induction and the averaging are
% [HAND-PROOF-UNREVIEWED, source J7]; the lower bound is thm:exact.
% Vacuity check: "K >= 3" -- at K = 2 the display's denominator e - 3 is
% negative and the exponential form is empty (W_2 itself still exists).
% "arbitrary size" -- this is what separates the theorem from
% thm:linear-exact; the hard family here is size-profiled on ALL sets with
% |S cap O| <= 1, and pays the strictly positive gap W_K - rho_K for it
% (the night-10 analysis shows the gap is unavoidable for globally
% size-profiled count-grid families, results/Q3_failure_point.md).
% "limit as n -> infinity" -- essential: at n < K + t* reverse greedy
% provably reads O off the leaking large sets and reaches ratio exactly 1
% (results/J7_grid_check.py, adversarial simulation), so no finite-n
% "<= W_K" statement holds wholesale.
% "O(nK) queries" -- superlinear budgets up to n^2/(2K^2(t*+K)^2) remain a
% candidate bound of the supplement (ledger T12), not a theorem.
% "deterministic ... randomized in expectation" -- both stated; no exact
% finite-n randomized claim.
% NOT to be confused: W_K here vs the W_m envelope of rem:exact-gap
% (different objects, ledger T7/T10d).
