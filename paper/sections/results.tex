...
$T$ satisfies $f(T)\le f(O^{\ast})/\eta$.  For every randomized algorithm
there is a fixed pair, again with error exactly $(\eta_u,\eta_o)$, on which
the expectation over the algorithm's randomness satisfies
$\mathbb E[f(T)]\le\bigl((1-\tfrac Kn)\tfrac1\eta+\tfrac Kn\bigr)
f(O^{\ast})$.
Conversely, exhaustive search over all $K$-subsets of predicted values
achieves $f(\hat S)\ge f(O^{\ast})/\eta$ on every instance.
\end{theorem}
The randomized expression is an upper bound on the randomized worst case; it
is not claimed to be the exact randomized value at finite $n$.  For $n<2K$
the same symmetric construction gives the sharper deterministic ceiling
$K/\bigl((2K-n)+(n-K)\eta\bigr)$, which interpolates between $1/\eta$ at
$n=2K$ and $1$ at $n=K$ and is attained by exhaustive search on the
LP-verified grid $K\le4$, $n\le7$ (general attainment is a conjecture).
% K9/H-E (ledger T8): ceiling value [VERIFIED-LP 84 grid points, error
% <= 3.4e-16, results/H_E_ceiling_small_n.py]; both hand arguments
% [HAND-PROOF-UNREVIEWED]; exhaustive attainment [VERIFIED-LP at grid,
% CONJECTURE general]; randomized value for n<2K stays open, and the
% product-only dependence is established for the symmetric family only.
% Status: [HAND-PROOF-UNREVIEWED], construction in Appendix
% \ref{app:ceiling}.  The exhaustive-search matching bound is the only
% surviving content of the removed R-step section.  K9 (J4 audit): the
% caveat sentence above and the remark below implement ledger card T8; the
% error-exactness of the construction is [VERIFIED-EXHAUSTIVE] per J4
% (253,220 all-pairs gains; script not delivered, recorded in REPORT).
...
% [VERIFIED-SYMBOLIC] sympy series at fixed eta, cross-checked against
% Richardson extrapolation to 1.3e-12; uniformity of the O(1/K^2) remainder
...
\begin{corollary}[Optimality within the greedy query budget]
...
$\mathcal A_{\mathrm{lin}}$ at $\min\{U_K(\eta),1/\eta\}$, so the best
...
whether any of it can be realized at finite $K$ is open.
\end{remark}
