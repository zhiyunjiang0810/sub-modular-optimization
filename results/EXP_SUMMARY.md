...
- 方法注记：viol=0 是结构性的（两覆盖函数增益非负，这把尺子在 E2 无信息量）；
  facebook 图 (d,d̃) 只存 top-50/步，η^path 系统性下估 —— **已于 2026-09-01（TASKS4 F1.2）修掉**：
  统计一律用全候选，240 条轨迹全量重算，K=30 的 η^path 合并中位数 71.7 → 284.0；
  ratio / η^sel / sign-viol 三列逐行不变（E5 的 η^sel 轴本来就不受影响）。
