...
> **第二晚 N5 修正**：完整闭式是 **1 + δ = max{a^τK/(K−τ), a^{1−τ}}²**，第二支来自 y 方向
> balanced 边（y = τ−1 处的 O-加边际），第一支占优 iff η ≥ 2 − 1/τ。本节当晚的全部测试点
> （τ=1 恒然；τ=2 时 η ≥ 1.5）都落在第一支区，故下面的单支公式在这些点上正确但不完整。
> 修正的 oracle：results/N5_delta_at_etahat.py（40 点）+ 主会话分歧点复核
> （K=4, η=1.2, τ=2：LP=0.595568=第二支，单支公式给 0.571193）。

对全部测试点（全格点：(n,K) ∈ {(8,3),(10,3),(10,4),(12,4)}；网格：K ∈ {3,4,6,8,12,16,24,32}，
n = 4K；η ∈ {1.5,2,3}；τ ∈ {1,2}；误差拆分 η_u=η_o=√η 与 (η_u,η_o)=(η,1) 两种）：

    min feasible δ 满足 1 + δ = (a^τ · K/(K−τ))²，a = 1 − 1/(ηK)（第一支区内）
...
## 覆盖范围说明（no silent caps）

relaxF 扫描覆盖 K ∈ {3,4,6,8,12,16,24} × n ∈ {4K, 8K}，除 (K=24, n=192, η=3) 的 4 个
配置：单个 LP 超过 58 分钟未解完（HiGHS 在该规模退化），按 45 分钟纪律放弃。
K=24、η=3 的数值以 n=96 行为准（K=3 处 n=4K 与 n=8K 的值差 < 1e-3，n 敏感性很低）。
figures/T2_relaxF_ratio.png 的 η=3 面板 n=8K 序列因此止于 K=16。
