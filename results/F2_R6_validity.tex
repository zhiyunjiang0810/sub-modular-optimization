...
Enumerate $O = \{o_{1}, \dots, o_{K}\}$, padding with elements of zero marginal
gain everywhere if $|O| < K$, and normalise $f(O) = 1$.
...
\eqref{eq:coh-ii}. This step uses \eqref{eq:coh-i}, that is R3(ii) reduces to
R3(i), and $\eta \ge 1$ so that the factor $1 - 1/\eta$ is nonnegative.
\end{proof}
...
The bands are used only through single-element gains at the greedy states, so
the same proofs go through verbatim with $\eta$ replaced by the trajectory error
$\etatr$, and, in Lemma~\ref{lem:pred} alone, by the selection error $\etasel$.
\end{remark}
