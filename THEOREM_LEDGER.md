...
此前 J5 到齐日 J5H1-J5H6：T8 统一 ceiling 重写并升级达到方向、T7 加 β_m 下包络与
...
- 禁止声称：~~有限 K、1 < η < K 时 greedy 在 𝒜_lin 内最优（[OPEN]）~~ **Q4 日更新：该项由 T10c 闭合**
  （n ≥ 4K⁵ 时允许声称，引用 T10c 并带其状态标签；改进空间 ≤ U_K−ρ_K = O(1/K²) 的表述改为
  "T10c 对旧天花板的改进幅度"）；
...
- Q4 日注记：本卡的单边天花板 min{U_K, 1/η} 被 T10c 的精确值 ρ_K 取代；计数链、n 量词反例、η=1 与 K=1
...
- 构造（双残差截断族，来源 Q4 外部并行审计）：j = max{0, K−⌊η⌋}，Q = q^j，δ = Q/(Kη)，C = k1/K；
  残差 r_x（几何段 q^x 到 x=j，斜率 δ 线性下降，[·]_+ 截断）与 h_x（同形，起点 (K−1)q^x/K）；
  F(x,0) = 1−r_x，F(x,y≥1) = 1−c_y h_x（c_y = (K−y)/(K−1)）；H = η_u G，H(x,0) = C−r_x−(η−1)h_x，
  H(x,y≥1) = C−η c_y h_x。公式与 n 无关：每个 n ≥ 2K 限制同一张表，F(K,0) = V_j = ρ_K 恒等。
- 关键点（为何绕开第十晚的 W 超额）：O-无关只在 |S| ≤ K、|S∩O| ≤ 1 处成立（H(x,y) = Ĥ_{x+y}，
  三段 Ĥ 见 app:greedybudget），这正是 transcript 归纳需要的全部；更大集合真实泄漏
  （K=3, η=3/2：H(6,0) = 61/48 ≠ 4/3 = H(5,1)，两者都是 6 元集）。N4/第十晚的 LP 把 O-无关强加到
  全部大小，才有 W = V_j + 超额；small-set-only 版 LP 在全部 18 个测试 (K,η,n)（含阶梯启动后的 n）
  恰等于 ρ_K [VERIFIED-LP，results/Q4_smallset_lp.py]。
- 状态：合法性（单调、submodular、band 四类边（y=0→1 下端 tight 实现 η_u，B-边 y≥1 与 O-边 y≥1 上端
  tight 实现 η_o）、归一化、两个校准端点在 x=0 处达到、prop:rigidity 轨迹逐步吻合、值 = ρ_K、
  (29) 相邻支恒等式与最小化分析）：13 条交付恒等式复跑 + 34 条自写分支不等式
  [VERIFIED-SYMBOLIC，results/Q4_gpt_check.py + results/Q4_symbolic_ineq.py]；
  独立精确电池 111 组（K ≤ 13，j=0 域、整数 η 双支、两个极端拆分）+ 交付电池 159 组/111,032 边
...
- 量词检验（逐词）：deterministic（随机版差 ε_n，单列）；≤ nK 查询（无预算时 thm:ceiling 只给 1/η）；
  ...
  在正增益边上达到，非上确界未达）。
- 禁止声称：任意大小查询的同预算精确值（泄漏是实质的，T12 仍 [OPEN]）；随机类有限 n 精确 = ρ_K；
  本定理证明了 F3 全局闭式或其 m* 规则的合法性（未证明；m* = ⌈ηK⌉−1 已被 Q2 精确反例驳倒，见 T12）；
  n < 4K⁵ 的任何最优性（M0 §2 反例）；(i) 的实例对全部算法统一（实例依赖算法与拆分）；
  第十晚 Q3 的 FAILED 结论作废（其对象是全局 O-无关族，结论仍真，见 results/Q3_failure_point.md 附录）。
...
- 禁止声称：c=0,1 的类（精确穷举反例：单查询算法可被逼到 0）；"O(c/K)"；~~有限 K 的同预算最优性 [OPEN]~~
  **Q4 日更新：Q = nK、查询大小 ≤ K 的类（𝒜_lin）的有限 K 精确最优性已由 T10c 闭合（n ≥ 4K⁵）**；
  对 n^c 尺度或任意大小查询的类，本禁止项照旧（仍 [OPEN]）。
